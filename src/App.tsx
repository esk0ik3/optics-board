...
export class OpticsLensUtil extends ShapeUtil<OpticsLensShape> {
  ...
  override component(shape: OpticsLensShape) {
    ...
    const getRadius = (chord: number, sagitta: number) => {
      const d = Math.max(0.1, Math.abs(sagitta))
      return (chord * chord) / (8 * d) + d / 2
    }

    let pathD = ''
    if (lensType === 'double-convex') {
      let d = Math.abs((150 / focalLength) * (w / 2))
      if (d > w) d = w
      let R = getRadius(h, d)
      pathD = `M ${w / 2} 0 A ${R} ${R} 0 0 1 ${w / 2} ${h} A ${R} ${R} 0 0 1 ${w / 2} 0 Z`
    } else if (lensType === 'double-concave') {
      let d = Math.abs((150 / focalLength) * (w / 3))
      if (d > w * 0.45) d = w * 0.45
      let R = getRadius(h, d)
      pathD = `M 0 0 L ${w} 0 A ${R} ${R} 0 0 0 ${w} ${h} L 0 ${h} A ${R} ${R} 0 0 0 0 0 Z`
    } else if (lensType === 'plano-convex') {
      let d = Math.abs((300 / focalLength) * (w * 0.8))
      if (d > w * 1.5) d = w * 1.5
      let chordX = w / 2 - d / 2
      let R = getRadius(h, d)
      pathD = `M ${chordX} 0 A ${R} ${R} 0 0 1 ${chordX} ${h} L ${chordX} 0 Z`
    } else if (lensType === 'plano-concave') {
      let d = Math.abs((300 / focalLength) * (w / 3))
      if (d > w * 0.7) d = w * 0.7
      let R = getRadius(h, d)
      let leftX = w * 0.1
      let rightX = w * 0.9
      pathD = `M ${leftX} 0 L ${rightX} 0 A ${R} ${R} 0 0 0 ${rightX} ${h} L ${leftX} ${h} Z`
    }
...
function CustomUI({ editor }: { editor: any }) {
  ...
  const [pos, setPos] = useState({ x: 10, y: 120 })
